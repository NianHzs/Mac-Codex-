...
try {
  ...
  foreach ($requiredCss in @(
    ...
    'html.codex-dream-skin.dream-theme-light [class~="group/application-menu-top-bar"] button',
    'left: var(--codework-dream-chrome-left, 50%)',
    'touch-action: none',
  ...
  if (-not $traySource.Contains(
    '[AllowEmptyCollection()][System.Windows.Forms.ToolStripItemCollection]$Items')) {
    throw 'Tray item construction rejects the valid empty collection used for the first menu entry.'
  }
  ...
  if (-not $startSource.Contains(
    "'--browser-id', `$cdpIdentity.BrowserId, '--timeout-ms', '90000'")) {
    throw 'Start does not allow enough time for the verified Codex shell to mount during a cold launch.'
  }
  if ($startSource -match '(?m)^Show-DreamSkinAiRelayDialog\s*$') {
    throw 'Normal Dream Skin startup must not open an AI relay dialog.'
  }
  if (-not $traySource.Contains('Start-Process -FilePath $powershell -ArgumentList $argumentLine -WindowStyle Hidden')) {
    throw 'Tray actions must launch PowerShell without opening a terminal window.'
  }
  $installerSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\install-dream-skin.ps1')
  if (-not $installerSource.Contains('$shortcut.Arguments = "-NoProfile -WindowStyle Hidden -ExecutionPolicy RemoteSigned') -or
    -not $installerSource.Contains('$restore.Arguments = "-NoProfile -WindowStyle Hidden -ExecutionPolicy RemoteSigned')) {
    throw 'Dream Skin launch shortcuts must start PowerShell without opening a terminal window.'
  }
  ...
  $themeVersionTest = Invoke-DreamSkinNative -FilePath $node.Path -ArgumentList @(
    (Join-Path $PSScriptRoot 'injector-theme-version.test.mjs'))
  if ($themeVersionTest.ExitCode -ne 0) { throw 'Injector theme-version regression test failed.' }
